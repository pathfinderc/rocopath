...
<b>路径规划</b>
框选点位 → 点击其中一个 → 自动设为起点 → 点击「规划路径」

<b>规划模式</b>
快速模式 → 最近邻算法（速度快）
质量模式 → OR-Tools 算法（结果更优）"""
...
<br>
<b>=== 路径规划 ===</b><br>
• <b>规划模式</b>切换：快速模式（最近邻）→ 速度快，适合少量点；质量模式（OR-Tools）→ 结果更优<br>
• 框选多个点位后，点击其中一个点位自动设为起点<br>
• 点击「规划路径」→ 生成路径，自动清空选中，可继续规划下一条<br>
• 每条路径起点显示序号，序号与导出文件名对应<br>
• 「清空选中状态」→ 仅清空选中，已规划路径保留<br>
• 「清空全部」→ 清空所有选中、路径和路径点<br>
<br>
<b>=== 添加路径 ===</b><br>
• 勾选「添加路径」→ 进入路径绘制模式<br>
• 左键点击空白处 → 添加蓝色菱形路径点，自动连线<br>
• 左键拖动路径点 → 移动位置，连线实时跟随<br>
• 中键拖动 → 在添加模式下平移地图<br>
• 取消勾选 → 路径完成，保留在画板上<br>
• 再次勾选 → 开始绘制新路径<br>
<br>
<b>=== 路径点 vs 资源点 ===</b><br>
• 资源点（彩色圆形）→ 不可删除、不可移动，来自游戏数据<br>
• 路径点（蓝色菱形）→ 可删除、可移动，用户自定义或导入<br>
• 两者均可参与路径规划和合并<br>
<br>
<b>=== 导入导出 ===</b><br>
• 文件 → 导入兼容格式（6.0之前）→ 导入旧版路径文件<br>
• 文件 → 导入兼容格式（6.0及之后）→ 导入新版路径文件<br>
• 文件 → 导出路径 → 导出兼容格式 → 导出为第三方导航格式<br>
• 导入的路径以路径点形式显示，不影响已有路线<br>
<br>
<b>=== 移除选中点 ===</b><br>
• 框选路径上要删除的点（含路径点）<br>
• 点击「移除选中」→ 删除选中点，自动将路径分割为多条<br>
• 路径点被移除后从画板消失；资源点仅从路线中移除<br>
